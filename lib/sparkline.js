...
function Sparkline(containerElt, o) {
  ...
  var useBars = (opts.useBars==undefined) ? false : opts.useBars;
  ...
  function stdDev(xyPoints) {
    var ys = xyPoints.map(function(xy) {
      return xy[1];
    });
    // var mu = mean(ys) = sum(ys)/ys.length
    // var ds = ys.each((y-mu)**2)
    // var sd = sqrt(sum(ds)/ds.length)
